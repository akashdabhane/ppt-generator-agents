...
- [x] 🐞 `requirements.txt` is UTF-16 and includes `pywin32` (Windows-only), so the Linux Docker build fails. Rewrite it as UTF-8 with only the direct dependencies.
- [x] 🐞 `frontend/Dockerfile` copies `.next/standalone`, but `next.config.ts` lacks `output: "standalone"`. Add it (and run `node server.js` in the runner).
...
- [x] 🐞 Add project-ownership checks to: `/slides/{n}/regenerate`, `DELETE /presentations/{id}`, `POST /documents/{id}/reindex` (all done, 404 for non-owners). Shared `get_owned_project` / `get_owned_presentation` / `get_owned_document` dependencies are in `deps.py`.
...
- [x] Load `SECRET_KEY` from env with no insecure default in production. Restrict CORS origins to `FRONTEND_URL`.
...
- [x] Embedding dimension: make it configurable and consistent across providers (Google `text-embedding-004` = 768 ≠ 1536).
- [x] Map `DocumentChunk` ↔ vector ids correctly for pgvector/Pinecone deletes (store `document_id` in metadata. Already there, but verify the Pinecone filter delete works on serverless).

## P2: Features & UX
- [x] Expose `language` and `tone` in the Generate form (the API and engine already use them).
- [x] Show the grounding summary (cited slides, removed claims) on the deck preview page. It's stored on the job today.
- [x] Persist `audience` (and `tone`/`language`) on `Presentation` so slide regeneration uses the deck's audience (needs a column → manual `ALTER` until Alembic).
...
- [x] Show upload errors (e.g. unsupported file type, 400) in the Documents tab; failed uploads are currently silent.
...
- [x] Add a `PATCH /projects/{id}` route (the schema already exists) and an edit UI.
...
- [x] Replace the 2 s `setInterval` polling with TanStack `refetchInterval`, or implement real SSE.

## P3: Architecture & quality
- [ ] Verify end to end with real keys: Gemini embeddings at 1536 dims into the existing Pinecone index, then a generated deck. Re-index documents indexed before D-015.
- [ ] Extractor tests for PDF/DOCX/XLSX with small fixture files.
- [ ] Store tokens in httpOnly cookies instead of localStorage (D-007).
- [x] Turn `rag/graph.py` into a real LangGraph `StateGraph`, or rename it to remove the "LangGraph" claim. (Renamed: `RAGPresentationEngine`, a plain pipeline.)
...
- [x] Set up Alembic migrations and stop relying on `create_all`.
...
- [x] Fix the README: says async SQLAlchemy/SSE/LangGraph; the root `.env.example` is actually at `backend/.env.example`.
